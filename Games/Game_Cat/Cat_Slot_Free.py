...
base_reel_stripes = Config.Const.C_Base_ReelSets[1][Const.C_ReelStrip]
...
class GameSlot(object):
    ...
    def paid_spin(self,totalbet):
        ...
        result[Const.R_Line] = get_line_combo(reel, totalbet, Config.Const.C_BetLine)
...
class Respin(object):
    ...
    def allocate(self):
        ...
        if len(self.unlock_pos_1) >= 1:
            self.pos_pro[self.unlock_pos_1[0]] = Config.Free_Respin_Reel[0]
        if len(self.unlock_pos_2) >= 1:
            self.pos_pro[self.unlock_pos_2[0]] = Config.Free_Respin_Reel[1]

        for pos_idx in self.unlock_pos_1[1:]:
            self.pos_pro[pos_idx] = Config.Free_Respin_Reel[2]
        for pos_idx in self.unlock_pos_2[1:]:
            self.pos_pro[pos_idx] = Config.Free_Respin_Reel[3]
